  ...
  ({ gap, speed, colors, variant, noFocus, style, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      if (pixelCanvasRef.current) {
        ...
        if (gap !== undefined) {
          pixelCanvas.setAttribute('data-gap', gap.toString())
        }
        if (speed !== undefined) {
          pixelCanvas.setAttribute('data-speed', speed.toString())
        }
        if (colors) {
          pixelCanvas.setAttribute('data-colors', colors.join(','))
        }
        if (variant) {
          pixelCanvas.setAttribute('data-variant', variant)
        }
        if (noFocus) {
          pixelCanvas.setAttribute('data-no-focus', '')
        }

        pixelCanvas.style.position = 'absolute'
        pixelCanvas.style.inset = '0'
        pixelCanvas.style.pointerEvents = 'none'
        pixelCanvas.style.width = '100%'
        pixelCanvas.style.height = '100%'

        pixelCanvasRef.current.appendChild(pixelCanvas)

        // Cleanup
        return () => {
          if (pixelCanvasRef.current?.contains(pixelCanvas)) {
            pixelCanvasRef.current.removeChild(pixelCanvas)
          }
        }
      }
